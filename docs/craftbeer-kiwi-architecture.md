...
- **`region` / `ownership_type` / `has_taproom` columns** (added 30 July) — `region` (text, nullable) is populated from the Brewers Guild of NZ's own regional groupings (e.g. `'Wellington, Wairarapa & Manawatū'`), sourced from a member listing Andy supplied, rather than a bespoke boundary scheme — directly resolves `automation-plan.md`'s `AP-PREREQ-3` (region boundary definition). Set on both brewery and bar rows, since region is a geographic fact independent of `venue_type`. `ownership_type` (text, nullable, check constraint `independent`/`corporate-subsidiary`) reuses the corporate-ownership findings from the 28 July NZBN backfill (Panhead and Tuatara are the two `corporate-subsidiary` rows; everything else independent) — `null` on bars. `has_taproom` (boolean, nullable) records whether a brewery has a public, visitable premises, checked individually via web search for all 24 current brewery rows rather than assumed — every one currently confirmed `true`, so the column currently adds no filtering value on the existing Wellington set but is expected to differentiate once national expansion introduces contract/distribution-only brands. All three default to `null` for new rows rather than a plausible-looking default, matching the existing `nzbn` pattern — see `craftbeer-kiwi-decisions.md` (`DEC-026`) for the full reasoning.
- **`parent_brewery_id` column** (added 31 July) — nullable `uuid`, self-referencing foreign key to `breweries.id` (`on delete set null`), linking a multi-site brand's satellite venues to their flagship row. Backfilled for six rows across four brand groups (Garage Project's three satellites, Panhead Tory Street, Double Vision's Island Bay site, Waitoa Victoria St); every other row stays `null`. Points at `id` rather than a text `brand_group` field specifically because `id` can't drift if a row's `name` or `address` is later edited — see `craftbeer-kiwi-decisions.md` (`DEC-031`) for the full reasoning, including why `nzbn` wasn't reused for this (already found unreliable for brand-grouping, `DEC-021`). Nothing in the frontend reads this yet — it exists to make the relationship queryable ahead of a future feature (grouped display, brand filtering, cross-links between a brand's sites), not to change current behaviour.
...
Each Supabase project (production and dev) maintains its own independent set of Edge Function secrets — nothing carries over automatically between them, unlike frontend `.env` files which at least share values by manual copy. As of 31 July, both `brewery-sync` and `brewery-discover` require:

- **`GOOGLE_PLACES_API_KEY`** — Places API key restricted to Places API only (Google Cloud Console, `craftbeer-kiwi-automation` project).
- **`MAPBOX_TOKEN`** — server-side geocoding token. Same value as the frontend's `VITE_MAPBOX_TOKEN`, but must be set separately — Edge Functions run outside Vite and never read `.env.local`.
- **The named secret key referenced by `auth: "secret:brewery_sync_v2"`** in both functions' code — created under Project Settings → API Keys → Secret keys, must be named exactly `brewery_sync_v2` to match what the code expects.

**Managed via CLI**, not manually per-value in the dashboard: `npx supabase secrets set KEY=value --project-ref <ref>` (overwrites cleanly if the key already exists — no delete-first step needed) and `npx supabase secrets list --project-ref <ref>` (shows a one-way digest hash per key, never the value itself — by design, so you can confirm a change actually took effect without re-exposing the secret).

**Calling these functions:** this project's `@supabase/server` auth pattern (`auth: "secret:<name>"`) expects the named secret key's value on the **`apikey` header** — not `Authorization: Bearer`, which is the more commonly assumed convention and caused real trial-and-error confusion before this was confirmed against the package's own docs on 31 July. The platform's own "Verify JWT with legacy secret" toggle (Settings tab, per function) should stay **off** for both functions, since they handle their own auth in code — leaving it on rejects requests before the function code ever runs, producing an identical-looking 401 to an actual credential problem.

**The `WARNING: Docker is not running` message during `supabase functions deploy` is expected and harmless** when deploying to Supabase's hosted cloud — Docker is only required for the *local* Supabase development stack, not for deploying against a real project.

**Dev project secrets were only set up for the first time 31 July** — previously only production had any of these configured, since neither function had ever been deployed to dev before that point. See `craftbeer-kiwi-decisions.md` (`DEC-033`) for the fuller story, including the specific debugging sequence that led to confirming the `apikey` header requirement.

### Automation infrastructure — Edge Functions

Two Supabase Edge Functions, deliberately kept as separate deployments rather than one combined function, for cost-tier isolation and failure blast-radius reasons (see `craftbeer-kiwi-decisions.md`). Both run against production. `brewery-discover` has also now been deployed to and dry-run tested against the dev project (31 July — see "Edge Function secrets" below and `craftbeer-kiwi-decisions.md` `DEC-033`); `brewery-sync` has not yet been deployed to dev. **Both are proven working end-to-end against production** (as of 27 July), after resolving the secret-key auth issue above.
