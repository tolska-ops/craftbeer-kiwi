...
**Resolved:** Option B was adopted and built — see `craftbeer-kiwi-architecture.md` for the live `craftbeer-kiwi-DEV` Supabase project, `.env.local` config, and `VITE_APP_ENV` indicator badge. This doc is kept as the historical record of the options considered and why.
...
- **What does Supabase's Free tier actually offer for point-in-time recovery / backups on the production project?** **Resolved 29 July** — nothing native. Daily backups and PITR are both Pro-plan-and-above only; Free tier gets neither. A paused project is recoverable (data frozen, not wiped), but a *deleted* project is not — no backups retained, no support escalation path. Mitigation: manual `supabase db dump` export, no automation needed at current volume. Full writeup and the export command now live in `craftbeer-kiwi-security.md` under "Data backups."
